package justaconcept.games.thelight;
...
public class TheLight extends PApplet {
	...
	for (String line : lines) {
	    ...
	    } else if (line_data[0].equals("hero")) {
		hero = new Hero(parseInt(line_data[1]), parseInt(line_data[2]));
	    } else if (line_data[0].equals("door")) {
	...
	readData("level" + Integer.toString(lvl_no) + ".txt");
	light = lights.get(0);
	blocked = false;
	dead = false;
	finished = false;
    ...
    final float my_rat = 320.0f / 480.0f;

    public int getDirection() {
	if (mouseY > round(my_rat * mouseX)) {
	    if (mouseY > 320 - round(my_rat * mouseX))
		return 2;
	    else
		return 3;
	} else if (mouseY > 320 - round(my_rat * mouseX))
	    return 4;
	else
	    return 1;
    }
